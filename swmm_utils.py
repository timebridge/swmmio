...
def drawNode(node, draw, options, rpt=None, dTime=None, xplier=1, svg=True):
	...
	if not svg:
		...
		color = 'rgb'+str(color)
		circle = draw.circle(xy, radius, fill = color, opacity=0.55)
...
def annotateMap (draw, model, model2=None, currentTstr = None, options=None, results={}, svg=True):
	...
	if not svg:
		modelSize = (draw.im.getbbox()[2], draw.im.getbbox()[3])
	else:
		modelSize = (draw['width'], draw['height'])
